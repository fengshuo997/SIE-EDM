...
def do_query_experiment(data=pd.DataFrame, neo4j_g=None, mysql_cursor=None, mysql_engine_data = [], mysql_database_name=None, ta=None, cta=None, column_structure=None, matchname=None, query_time=None, datasize=120000,sp=10):
    ...
    column_structure_og = column_structure
    ...
    for n in datasize_sample:
        ...
        import pymysql
        conn = pymysql.connect(port=3306, user=mysql_engine_data[0], password=mysql_engine_data[1], database=mysql_engine_data[2])
        ...
        tagetrow = data.loc[(data['ENERGY_SOURCE'] == matchname2) & (data['MEASURE'] == matchname1)]
        if n==0:
        ...
        column_structure = column_structure_og

        # container.exec_run(f"rm -rf '/data/'")
        # container.restart()
        # time.sleep(10)
        # driver = GraphDatabase.driver("bolt://localhost:7687", auth=('neo4j', '9eaJjkwMUv3GRGn'))
        ...
        neo4j_database_size = neo4j_databasesize_count(neo4j_g)
        ...
        # column_structure = [['ENERGY_SOURCE', 'MEASURE', 'TIMESTAMP'], ['INTERVAL', 'LOCATION', 'UNIT_OF_MEASURE', 'DATE', 'VALUE']]
        ...
        # database, stat = container.get_archive(container_file_path)
        # with open(local_destination+'_single', 'wb') as f:
        #     for chunk in database:
        #         f.write(chunk)
        #     f.seek(0, 2)
        #     print(f.tell())
